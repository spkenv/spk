...
use spk_schema_foundation::name::{OptNameBuf, PkgNameBuf};
...
impl WhenBlock<VarRequest<String>> {
    /// Determine if this when block is satisfied by the
        ...
        match self {
            ...
            Self::Pkg(req) => {
                let Some(resolved) = build_env.get_member(req.pkg.name()) else {
                    return ConditionOutcome::disabled(format!(
                        "pkg: {} is not present in the build environment",
                        req.pkg.name()
                    ));
                };
            ...
            Self::Var(req) => {
                ...
                let Some(value) = value else {
                    return ConditionOutcome::disabled(format!(
                        "var: {} is not present in the build environment",
                        req.var
                    ));
                };
                if value != &req.value {
...
/// The result of checking a when condition
pub type IsSatisfied = Result<(), NotSatisfiedReason>;

#[derive(Debug, strum::Display)]
pub enum NotSatisfiedReason {
    #[strum(to_string = "package '{0}' is not present in the build environment")]
    PackageNotInBuildEnv(PkgNameBuf),
    #[strum(to_string = "var '{0}' has no value in the build environment")]
    VarNotInBuildEnv(OptNameBuf),
    #[strum(to_string = "needed {needed}, but the value was {got}")]
    WrongVarValue { needed: String, got: String },
    #[strum(to_string = "the request for {pkg}, was not met: {reason}")]
    Incompatible {
        pkg: PkgRequest,
