...
use spk_schema_ident::{PkgRequest, Satisfy, VarRequest};

use super::RecipeOption;
use crate::{BuildEnv, Result};
...
impl RecipeOptionList {
    ...
    pub fn to_package_options<E>(&self, build_env: &E) -> Result<Vec<super::PackageOption>>
    where
        E: BuildEnv,
        E::Package: Satisfy<PkgRequest> + Satisfy<VarRequest>,
    {
        let mut options = Vec::with_capacity(self.len());
        for recipe_opt in self.iter() {
