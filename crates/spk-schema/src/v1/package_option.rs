...
impl VarOption {
    ...
    pub fn to_request(&self, given: Option<&String>) -> Option<VarRequest> {
        self.var.value(given).map(|value| VarRequest {
            ...
            value: spk_schema_ident::PinnableValue::Pinned(value.as_str().into()),
            description: None,
        })
    }
...
impl Satisfy<VarRequest<String>> for VarOption {
    fn check_satisfies_request(&self, var_request: &VarRequest<String>) -> Compatibility {
