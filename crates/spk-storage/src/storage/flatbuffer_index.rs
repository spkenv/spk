...
impl FileLock {
    ...
    pub async fn new<P: AsRef<Path>>(
        ...
        let mut lock_data;
        let held_for = match tokio::fs::File::open(&lock_file).await {
            Ok(mut f) => {
                // Get contents of the lock file
                let mut buffer: String = Default::default();
                lock_data = match f.read_to_string(&mut buffer).await {
                    Ok(_) => buffer,
                    Err(err) => {
                        format!("Unknown due to error ({err})")
                    }
                };

                // Work out, how long the lock has been held
        ...
        if !self.has_been_deleted {
            if let Err(err) = std::fs::remove_file(&self.lock_file) {
                ...
                if err.kind() != std::io::ErrorKind::NotFound {
                    ...
                    // The file was not found, so record that it does
                    // not exist anymore.
...
impl Drop for FileLock {
    fn drop(&mut self) {
        ...
        {
            // Can only warn about the problem here. This cannot
            // return an error. But the unlock() method will have sent
            // it to sentry, if that feature is enabled.
            tracing::warn!("{err}");
        }
    }
}
