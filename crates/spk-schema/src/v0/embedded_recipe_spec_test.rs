...
fn test_sources_relative_to_spec_file(tmpdir: tempfile::TempDir) {
        ...
        .unwrap()
    {
        ...
        panic!("expected spec to be a Spec::V0Package");
    }
}
...
fn test_yaml_error_context(#[case] yaml: &str, #[case] expected: &str) {
    ...
    let err = EmbeddedRecipeSpec::from_yaml(yaml).expect_err("expected yaml parsing to fail");
...
fn test_build_options_respect_components() {
    let spec: EmbeddedRecipeSpec = serde_yaml::from_str(
        r#"
        pkg: test-pkg
        build:
          options:
