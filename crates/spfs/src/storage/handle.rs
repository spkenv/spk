...
pub enum RepositoryHandle {
    ...
    FallbackProxyWithMaybeRenders(Box<super::fallback::FallbackProxy<MaybeRenderStore>>),
    FallbackProxyWithRenders(Box<super::fallback::FallbackProxy<RenderStore>>),
    FallbackProxyWithoutRenders(Box<super::fallback::FallbackProxy<NoRenderStore>>),
...
impl RepositoryHandle {
    ...
    pub fn try_as_tag_mut(&mut self) -> Result<&mut dyn TagStorageMut> {
        match self {
            ...
            RepositoryHandle::FallbackProxyWithMaybeRenders(repo) => Ok(&mut **repo),
            RepositoryHandle::FallbackProxyWithRenders(repo) => Ok(&mut **repo),
            RepositoryHandle::FallbackProxyWithoutRenders(repo) => Ok(&mut **repo),
...
macro_rules! each_variant {
    ($repo:expr, $inner:ident, $ops:tt) => {
        match $repo {
            ...
            RepositoryHandle::FallbackProxyWithMaybeRenders($inner) => $ops,
            RepositoryHandle::FallbackProxyWithRenders($inner) => $ops,
            RepositoryHandle::FallbackProxyWithoutRenders($inner) => $ops,
...
impl TagStorageMut for RepositoryHandle {
    fn try_set_tag_namespace(
        ...
        match self {
            ...
            RepositoryHandle::FallbackProxyWithMaybeRenders(repo) => {
                repo.try_set_tag_namespace(tag_namespace)
            }
            RepositoryHandle::FallbackProxyWithRenders(repo) => {
                repo.try_set_tag_namespace(tag_namespace)
            }
            RepositoryHandle::FallbackProxyWithoutRenders(repo) => {
                repo.try_set_tag_namespace(tag_namespace)
            }
