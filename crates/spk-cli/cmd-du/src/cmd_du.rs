...
impl<T: Output> Du<T> {
    ...
    fn du_walk<'a>(
        ...
        Box::pin(try_stream! {
            ...
            let mut current_repo_name = Arc::new(String::from(""));
            let mut pkg_name = Arc::new(PkgNameBuf::try_from("a").unwrap());
            ...
            while let Some(item) = traversal.try_next().await? {
                match item {
                    ...
                    RepoWalkerItem::File(file) => {
                        ...
                        let mut du = PackageDiskUsage::new((*pkg_name).clone(), (*current_repo_name).clone());
