...
use rstest::rstest;
use spk_schema_foundation::{FromYaml, IsDefault, option_map, pkg_name};

use super::{AutoHostVars, RecipeBuildSpec};
use crate::v0::UncheckedRecipeBuildSpec;
...
fn test_build_spec_with_host_opt_value(#[case] value: &str, #[case] expected_result: bool) {
    ...
    let res = serde_yaml::from_str::<UncheckedRecipeBuildSpec>(&format!(
        "{{
...
fn test_build_spec_with_host_opt_contains_expected_names(
    ...
    let res: serde_yaml::Result<RecipeBuildSpec> = serde_yaml::from_str(&format!(
        "{{
        auto_host_vars: {value},
        options: [{{pkg: \"my-pkg\"}}],
    }}"
...
fn test_variants_may_have_a_build() {
    let res = serde_yaml::from_str::<UncheckedRecipeBuildSpec>(
        r#"{
        options: [{pkg: "my-pkg"}],
...
fn test_variants_must_be_unique() {
    ...
    let res: serde_yaml::Result<RecipeBuildSpec> = serde_yaml::from_str(
        r#"{
...
fn test_yaml_error_unchecked_to_checked() {
  ...
  variants:
    ...
    - opt: a
"#;
    let err = Vec::<RecipeBuildSpec>::from_yaml(YAML).expect_err("expected yaml parsing to fail");
    let expected = r#"
 1 | - options:
...
options:
  - pkg: pkg:comp2
"#;
    let res1 = serde_yaml::from_str::<RecipeBuildSpec>(yaml1).unwrap();
    let res2 = serde_yaml::from_str::<RecipeBuildSpec>(yaml2).unwrap();
    let build_id1 = res1
