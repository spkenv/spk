...
def test_request_merging() -> None:
    ...
    assert str(decision.get_merged_request("my-pkg").pkg) == "my-pkg/1.0.0"  # type: ignore

    decision.add_request(api.Request.from_dict({"pkg": "my-pkg/1.0/src"}))

    assert str(decision.get_merged_request("my-pkg").pkg) == "my-pkg/1.0.0/src"  # type: ignore
