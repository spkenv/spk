...
async fn test_impossible_requests_checker_validate_pkg_requests_include_embedded_impossible() {
    ...
    unresolved_requests.insert(
        same_pkg_request.pkg_request.pkg.name.clone(),
        same_pkg_request.clone(),
    );
...
async fn test_impossible_requests_checker_validate_pkg_requests_no_requirements() {
    ...
    let mut unresolved_requests: HashMap<PkgNameBuf, PkgRequestWithOptions> = HashMap::new();
    unresolved_requests.insert(request.pkg_request.pkg.name.clone(), request.clone());
...
async fn test_impossible_requests_checker_with_uncombinable_requests() {
    ...
    let mut unresolved_requests: HashMap<PkgNameBuf, PkgRequestWithOptions> = HashMap::new();
    unresolved_requests.insert(request.pkg_request.pkg.name.clone(), request);
