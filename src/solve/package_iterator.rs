            ...
            if !self.builds_map.contains_key(version) {
                self.builds_map.insert(
                    ...
                    Arc::new(tokio::sync::Mutex::new(
                        RepositoryBuildIterator::new(pkg.clone(), repo.clone()).await?,
                    )),
                );
            }
...
impl RepositoryPackageIterator {
    ...
    async fn build_version_map(
        &self,
    ) -> Result<HashMap<api::Version, Arc<storage::RepositoryHandle>>> {
        let mut version_map = HashMap::default();
        for repo in self.repos.iter().rev() {
            for version in repo.list_package_versions(&self.package_name).await? {
