...
impl Solver {
    ...
    async fn get_iterator(
        ...
        node.set_iterator(package_name.clone(), &iterator).await;
        iterator
    }
    ...
    async fn step_state(&mut self, node: &mut Node) -> Result<Option<Decision>> {
        ...
        loop {
            ...
            let builds = if !builds.lock().await.is_sorted_build_iterator() {
                let builds = Arc::new(tokio::sync::Mutex::new(
                    SortedBuildIterator::new(node.state.get_option_map(), builds.clone()).await?,
                ));
...
pub struct SolverRuntime {
    ...
    history: Vec<Arc<tokio::sync::RwLock<Node>>>,
    current_node: Option<Arc<tokio::sync::RwLock<Node>>>,
...
impl SolverRuntime {
    ...
    async fn take_a_step_back(
        history: &mut Vec<Arc<tokio::sync::RwLock<Node>>>,
    ...
    pub fn iter(&mut self) -> impl Stream<Item = Result<(Node, Decision)>> + Send + '_ {
        stream! {
            'outer: loop {
