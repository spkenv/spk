...
use crate::{BuildSpec, Error, Opt, Result, ValidationSpec, Variant, v0};

#[cfg(test)]
#[path = "./recipe_build_spec_test.rs"]
mod recipe_build_spec_test;
...
///
/// This represents the `build` section of a package recipe. Once built,
/// [`crate::BuildSpec`] is used.
...
impl IsDefault for RecipeBuildSpec {
    fn is_default(&self) -> bool {
        self == &Self::default()
    }
}

impl From<v0::EmbeddedBuildSpec> for RecipeBuildSpec {
...
/// Convert from a BuildSpec to a RecipeBuildSpec.
///
...
impl TryFrom<UncheckedRecipeBuildSpec> for RecipeBuildSpec {
    type Error = crate::Error;

    fn try_from(bs: UncheckedRecipeBuildSpec) -> std::result::Result<Self, Self::Error> {
        let bs = unsafe {
...
impl<'de> Deserialize<'de> for RecipeBuildSpec {
    fn deserialize<D>(deserializer: D) -> std::result::Result<Self, D::Error>
    where
        D: serde::Deserializer<'de>,
    {
        UncheckedRecipeBuildSpec::deserialize(deserializer)
            .and_then(|bs| bs.try_into().map_err(serde::de::Error::custom))
    }
}

/// A [`RecipeBuildSpec`] that can be deserialized more forgivingly.
///
...
pub(crate) struct UncheckedRecipeBuildSpec(RecipeBuildSpec);

impl UncheckedRecipeBuildSpec {
    /// Unwrap this instance into a true validated [`RecipeBuildSpec`].
    ///
    ...
    pub unsafe fn into_inner(self) -> RecipeBuildSpec {
        self.0
    }
}

impl<'de> Deserialize<'de> for UncheckedRecipeBuildSpec {
    fn deserialize<D>(deserializer: D) -> std::result::Result<Self, D::Error>
    where
        D: serde::Deserializer<'de>,
    {
        ...
        impl<'de> serde::de::Visitor<'de> for UncheckedBuildSpecVisitor {
            type Value = UncheckedRecipeBuildSpec;
                ...
                A: serde::de::MapAccess<'de>,
            {
                ...
                let mut unchecked = RecipeBuildSpec::default();
                ...
                Ok(UncheckedRecipeBuildSpec(unchecked))
            }
        }
