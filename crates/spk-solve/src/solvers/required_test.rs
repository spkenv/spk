...
use spk_schema::{Package, Request, opt_name};
use spk_solve_macros::{make_build, make_repo, request};

use super::{resolvo_solver, run_and_print_resolve_for_tests, step_solver, version_ident};
...
async fn requested_required_var_is_strongly_inherited() {
    ...
    mypkg
        ...
        .find(|req| {
            ...
            var_req.var.as_str() == "mylib.namespace_style"
                && var_req.value.as_pinned() == Some("major_minor")
        })
