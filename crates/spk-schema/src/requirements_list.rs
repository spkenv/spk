            ...
            match request {
                PinnableRequest::Pkg(pkg_request) => {
                    match resolved_by_name.get(pkg_request.pkg.name()) {
                        ...
                        Some(resolved) => {
                            Some(pkg_request.render_pin(resolved).map_err(Into::into).map(PinnedRequest::Pkg))
                        }
                    }
                }
...
impl From<&RequirementsList<PinnedRequest>> for RequirementsList<RequestWithOptions> {
    fn from(requirements: &RequirementsList<PinnedRequest>) -> Self {
...
{
    fn from((options, requirements): (I, &RequirementsList<PinnedRequest>)) -> Self {
...
/// Convert an iterable of [`PinnedRequest`] into an iterator of
/// [`RequestWithOptions`], considering the provided options in scope.
///
/// Since the iterable of [`PinnedRequest`] must be re-iterable, it is provided
...
where
    ...
    F: Fn() -> R,
    R: IntoIterator<Item = &'i PinnedRequest>,
    <R as IntoIterator>::IntoIter: 'i,
{
    ...
    for req in requirements().into_iter() {
        ...
        {
            btree_map::Entry::Vacant(e) => {
                e.insert(PkgRequestOptionValue::Complete(var_req.value.to_string()));
            }
            btree_map::Entry::Occupied(e) => {
                if e.get().value() != &*var_req.value {
                    todo!(
    ...
    requirements().into_iter().map(move |req| match req {
        PinnedRequest::Pkg(pkg_req) => RequestWithOptions::Pkg(PkgRequestWithOptions {
            pkg_request: pkg_req.clone(),
            options: index.get(pkg_req.pkg.name()).cloned().unwrap_or_default(),
        }),
        PinnedRequest::Var(var_req) => RequestWithOptions::Var(var_req.clone()),
    })
}
