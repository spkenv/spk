...
| embeded      | _List[[Spec](#Spec)]_       | A list of packages that come bundled in this one |
...
| build     | The build string, should not be specified in a spec file as it is generated by the system at build time. Digests are calculated based on the package build options, and there are two special values `src` and `embeded` for source packages and embeded packages, respectively |
