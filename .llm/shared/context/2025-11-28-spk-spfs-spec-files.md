...
## Quickstart (dev)
- **Prereqs**: Rust toolchain, `cargo build -p spfs-cli`, local spfs repo initialized, Linux permissions (`make setcap bindir=target/debug`) or WinFsp on Windows.
- **Setup**:
...
5. **Mounting** (`runtime/live_layer.rs`, `env.rs`): during runtime init, live layers are validated and bind-mounted over `/spfs` after overlay stack is mounted.
...
- **Spec files**: Must live on local filesystem accessible to CLI (remote paths not supported). Run-specs require `api: spfs/v0/runspec`; live layers require `api: spfs/v0/livelayer`.
...
## Open questions
- Are there additional spec APIs (e.g., future `spfs/v1/*`) under development? Inspect `crates/spfs/src/runtime/spec_api_version.rs` for upcoming variants.
- How do Windows runtimes treat live layer files (WinFsp backend)? Review `crates/spfs/src/runtime/winfsp.rs` to confirm parity.
- Should run-spec files allow relative includes? Currently absolute paths are required—evaluate security vs usability trade-offs.
