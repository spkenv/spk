...
use super::{InclusionPolicy, PinnedRequest, PreReleasePolicy};
use crate::FromYaml;
...
fn test_prerelease_policy_restricts(
    ...
    let mut a = serde_yaml::from_str::<PinnedRequest>(request_a)
        .unwrap()
        .pkg()
        .expect("expected pkg request");
    let b = serde_yaml::from_str::<PinnedRequest>(request_b)
        .unwrap()
        .pkg()
        .expect("expected pkg request");
...
fn test_prerelease_policy_contains(
    ...
    let a = serde_yaml::from_str::<PinnedRequest>(request_a)
        .unwrap()
        .pkg()
        .expect("expected pkg request");
    let b = serde_yaml::from_str::<PinnedRequest>(request_b)
...
fn test_inclusion_policy() {
    let mut a =
        serde_yaml::from_str::<PinnedRequest>("{pkg: something, include: IfAlreadyPresent}")
            .unwrap()
            .pkg()
            .expect("expected pkg request");
    let b = serde_yaml::from_str::<PinnedRequest>("{pkg: something, include: Always}")
        .unwrap()
        .pkg()
        .expect("expected pkg request");
...
fn test_compat_and_equals_restrict() {
    let mut a = serde_yaml::from_str::<PinnedRequest>("{pkg: something/Binary:1.2.3}")
        .unwrap()
        .pkg()
        .expect("expected pkg request");
    let b = serde_yaml::from_str::<PinnedRequest>("{pkg: something/=1.2.3}")
        .unwrap()
        .pkg()
        .expect("expected pkg request");
...
fn test_inclusion_policy_and_merge(
    ...
    let mut a = serde_yaml::from_str::<PinnedRequest>(a)
        .unwrap()
        .pkg()
        .unwrap();
    let b = serde_yaml::from_str::<PinnedRequest>(b)
        .unwrap()
        .pkg()
        .unwrap();
...
fn test_deserialize_value_or_pin() {
    let res = serde_yaml::from_str::<PinnedRequest>("{var: python.abi/cp27m}");
    assert!(res.is_ok(), "should allow regular name/value");

    let res = serde_yaml::from_str::<PinnedRequest>("{var: python.abi, fromBuildEnv: true}");
    assert!(res.is_err(), "should not allow no value");

    let res = serde_yaml::from_str::<PinnedRequest>("{var: python.abi/cp27m, fromBuildEnv: true}");
    assert!(
        res.is_ok(),
        "should allow value and pin [which is ignored for this type]"
    );

    let res = serde_yaml::from_str::<PinnedRequest>("{var: python.abi}");
    assert!(
        res.is_err(),
        "should not allow omitting value without pin [which is ignored for this type]"
    );
}
...
fn test_pkg_request_pin_rendering(
    ...
    let req = serde_yaml::from_str::<PinnedRequest>(&format!("{{pkg: test, fromBuildEnv: {pin}}}"))
        .unwrap()
        .pkg()
...
- var: hello
  pkg: hello
"#;
    let err = Vec::<PinnedRequest>::from_yaml(YAML).expect_err("expected yaml parsing to fail");
    let expected = r#"
   | - var: os/linux
 2 | - var: hello
...
fromBuildEnv: true
"#;
    let err = PinnedRequest::from_yaml(YAML).expect_err("expected yaml parsing to fail");
    let expected = r#"
 1 | pkg: python/3
...
- pkg: python
  fromBuildEnv: API
"#;
    let reqs = Vec::<PinnedRequest>::from_yaml(YAML).expect("expected yaml parsing to succeed");
    let pins: Vec<_> = reqs
        .into_iter()
