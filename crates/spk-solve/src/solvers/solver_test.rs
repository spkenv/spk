...
/// for unit test debugging and inspection.
async fn run_and_log_resolve_for_tests(solver: &mut SolverImpl) -> Result<Solution> {
    match solver {
        SolverImpl::Og(solver) => {
            let formatter = DecisionFormatterBuilder::default()
                .with_verbosity(100)
...
fn og_solver() -> SolverImpl {
    SolverImpl::Og(StepSolver::default())
}

fn cdcl_solver() -> SolverImpl {
    SolverImpl::Cdcl(ResolvoSolver::default())
}
...
fn test_error_frequency() {
    ...
    solver.increment_error_count(step::ErrorDetails::Message(an_error.clone()));
...
fn test_error_frequency_get_message_for_couldnotsatisfy_error() {
    ...
    solver.increment_error_count(step::ErrorDetails::CouldNotSatisfy(
        request.pkg.to_string(),
        request.get_requesters(),
    ));
    let errors: &std::collections::HashMap<String, step::ErrorFreq> = solver.error_frequency();
