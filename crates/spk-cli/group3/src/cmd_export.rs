...
pub struct Export {
    ...
    pub solve_requests: Vec<String>,

    /// The package to export (single-package mode)
    #[clap(name = "PKG", required_unless_present = "solve_requests")]
    pub package: Option<String>,

    /// The file to export into for single-package mode
...
impl Export {
    ...
    fn derive_solve_filename(requests: &[String]) -> std::path::PathBuf {
        let base = requests
            .first()
...
impl Run for Export {
    ...
    async fn run(&mut self) -> Result<Self::Output> {
        if self.filename.is_some() && self.output_file.is_some() {
            bail!("Specify either a positional FILE or --file, not both");
        }
        if !self.solve_requests.is_empty() && (self.package.is_some() || self.filename.is_some()) {
            bail!("--solve mode does not accept positional PKG/FILE arguments");
        }

        if self.solve_requests.is_empty() {
            ...
            let package = self
                .package
                .as_ref()
                .expect("clap should require PKG when --solve is not provided");
            let pkg = self
                .requests
        ...
        let (requests, extra_options) = self
            .requests
            .parse_requests(&self.solve_requests, &self.options, solver.repositories())
            .await?;
        ...
        let filename = self
            .output_file
            .clone()
            .unwrap_or_else(|| Self::derive_solve_filename(&self.solve_requests));
        let res =
...
impl CommandArgs for Export {
    fn get_positional_args(&self) -> Vec<String> {
        ...
        if !self.solve_requests.is_empty() {
            return self.solve_requests.clone();
        }
